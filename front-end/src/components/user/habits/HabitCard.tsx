...
const HabitCard: React.FC<Props> = ({ 
  habit, onEdit, onArchiveToggle, onDelete, isArchivedView, isProcessing 
}) => {
  ...
  return (
    ...
    >
      <div className="habit-card__actions">
        <button 
          className="habit-card__action-btn habit-card__action-btn--edit"
        ...
        <button 
          ...
          title={isArchivedView ? 'Restore to Active' : 'Archive Habit'}
          disabled={isProcessing}
        >
        ...
        <button 
          ...
          onClick={() => {
            if(window.confirm(`Permanently delete "${habit.title}"? This cannot be undone.`)) {
              onDelete(habit.id);
            }
          }}
      ...
      <div className="habit-card__header">
        ...
        {habit.category && (
          <span 
            className="habit-card__category-pill"
            style={{ background: `${habit.category.color}20`, color: habit.category.color, borderColor: `${habit.category.color}40` }}
          >
            <i className={`${habit.category.icon} me-1`}></i>
            {habit.category.name}
          </span>
        )}
      </div>
      ...
      <div className="habit-card__meta">
        ...
        {isArchivedView && habit.archived_at && (
          <span className="habit-card__archived-badge">
            <i className="fas fa-box-archive me-1"></i>
            Archived {new Date(habit.archived_at).toLocaleDateString()}
          </span>
        )}

        <span className="habit-card__type-badge">
