...
const CategoryFormModal: React.FC<Props> = ({ 
  ...
  const footer = (
    <div className="d-flex gap-2 w-100 justify-content-end">
      ...
      <button type="submit" form="category-form" className="btn btn-primary" style={{ background: 'var(--gradient-btn)', border: 'none' }} disabled={isLoading}>
        {isLoading ? <><span className="spinner-border spinner-border-sm me-2" /> Saving...</> : (initialData ? 'Update Category' : 'Create Category')}
      </button>
    </div>
  );
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={initialData ? 'Edit Category' : 'New Category'} footer={footer}>
      <form id="category-form" onSubmit={handleSubmit(onFormSubmit)} className="d-flex flex-column gap-3">
        ...
        <div className="settings-form__group mb-0">
          ...
          <input type="text" className={`settings-form__input ${errors.name ? 'is-invalid' : ''}`} placeholder="e.g., Health & Fitness" {...register('name')} />
        ...
        <div className="settings-form__group mb-0">
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="flex-grow-1">
              <ColorPicker value={watchedColor} onChange={(val) => setValue('color', val, { shouldValidate: true })} />
            </div>
            <div className="category-form__color-preview" style={{ backgroundColor: isValidHex ? watchedColor : '#cbd5e1', marginBottom: '0' }} />
          </div>
        ...
        <div className="settings-form__group mb-0">
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="flex-grow-1">
              <IconPicker value={watchedIcon} onChange={(val) => setValue('icon', val, { shouldValidate: true })} />
            </div>
            <div className="category-form__icon-preview" style={{ color: isValidHex ? watchedColor : 'var(--text-main)', marginBottom: '0' }}>
