...
*   **OAuth Fallback & State Management:** If the browser blocks the OAuth popup window, the system must gracefully fall back to a full-page redirect. The system must securely store the OAuth `state` parameter in `sessionStorage` to pass it to the callback page.
...
### Key Components
*   **`OAuthButtons`:** Manages the Popup flow, stores the `state` in `sessionStorage`, listens for `postMessage` events from the popup, and implements the full-page redirect fallback if `window.open` is blocked.
...
| `POST` | `/api/auth/verify-email?{qs}` | `authService.verifyEmail` | **Yes** | Verifies email using raw signed query string. |
...
| `POST` | `/api/auth/oauth/{provider}/callback` | `authService.handleOAuthCallback` | **Yes** | Exchanges OAuth code and state for user session. |
...
### OAuth Popup Flow (with Fallback & State Management)
...
3.  Frontend stores `state` in `sessionStorage` keyed by provider.
...
6.  **If Popup Opens:** User authenticates. Provider redirects to frontend `OAuthCallbackPage` with `?code=xxx`.
7.  `OAuthCallbackPage` extracts `provider` from URL params, retrieves `state` from `sessionStorage`, and sends `POST /oauth/{provider}/callback` with `{ code, state }`.
8.  On success, `OAuthCallbackPage` sends `postMessage('oauth-success')` to `window.opener`, clears `sessionStorage`, and closes itself.
...
*   **HMAC Signature Corruption (Handled):** Reconstructing URL parameters via JS objects scrambles key order, invalidating the signature. **Fix:** Passing `rawQueryString` directly from `window.location.search` as a query string on the POST request.
...
*   **Component (`OAuthButtons`):** Mock `window.open` to return `null` and assert that `window.location.href` is set (fallback). Mock `postMessage` listener to assert `currentUser` invalidation. Verify `sessionStorage` is populated with `state`.
...
*   **OAuth State Management:** Because the backend uses a Cache-based state mechanism for CSRF protection, the frontend must securely store the `state` returned by `getOAuthRedirect` in `sessionStorage` and forward it to the backend during the callback. This ensures the flow remains completely stateless regarding Laravel sessions.