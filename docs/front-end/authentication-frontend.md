...
It handles credential authentication, OAuth2 flows (via Popup with fallback), email verification (preserving cryptographic signatures and ensuring seamless post-verification UX transitions), and password recovery, all while maintaining strict client-side validation, robust protection against common web vulnerabilities, and intelligent state persistence and fallback mechanisms for pre-verification flows.
...
*   **`VerifyEmailPage`:** A multi-state component. State 1: Displays a "Check your inbox" UI post-registration, utilizing a DRY `renderResendUI` helper. If `pendingEmail` is missing from state, it renders a fallback input field so the user can manually provide their email. State 2: Automatically extracts signed URL parameters and sends them as a **JSON body** when navigated to via email link. It explicitly `await`s the verification mutation and tracks a local `isVerificationSuccess` state to render a clear success UI with a dashboard redirect link, preventing infinite loading spinners.
