...
class HabitLogController extends Controller
{
    ...
    {
        $log = $this->habitLogService->logCompletion($request->user(), $id, $request->validated());
        return $this->successResponse(new HabitLogResource($log), 'Habit logged successfully.', 201);
    }

    public function update(UpdateHabitLogRequest $request, HabitLog $habit_log): JsonResponse
    {
        $log = $this->habitLogService->updateLog($habit_log, $request->validated());
        return $this->successResponse(new HabitLogResource($log), 'Habit log updated successfully.');
    }
